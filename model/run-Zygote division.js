...
const fs = require('fs');
const file_name = "MCSOptimizationICM-TE-Lumen"
const ext = ".txt"
	...
	if (sim.time == (fusion_time + 1)){
		...
		for(let i of this.C.cellIDs()){
			if(this.C.cellKind(i) == 4){	
				...
				//this.toggleAnim()
				let data = String([this.C.conf.seed, sim.time, "Shape ICM", this.C.cells[i].distortion])
				this.dataLogger(filepath_values, data)

				let data2 = String([this.C.conf.seed, sim.time, "Disconnectedness ICM", this.C.cells[i].disconnectedness])
				this.dataLogger(filepath_values, data2)

				let data3 = String([this.C.conf.seed, sim.time, "Volume growth % lumen",  this.C.conf["VOLSTEP"][2]])
				this.dataLogger(filepath_values, data3)

				let data4 = String([this.C.conf.seed, sim.time, "Volume lumen", this.C.conf["V"][2]])
