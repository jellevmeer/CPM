...
module.exports = function model(jEpi_Epi, jPre_Pre, seed, perimeterSize){
	...
	*/
	let config = {
		...
		conf : {
			...
			J : [ 
				...
				[120, 1, 65, 20, 5, jEpi_Epi, 10],							// Epiblast
				[120, 1, 30, 25, 10, 10, jPre_Pre]							// PrE
			],
		...
		//SHOWBORDERS : [true, false, false, false],			// Should cellborders be displayed?
			...
			LOGRATE : 100000								// Output stats every <LOGRATE> MCS.
	...
	// Global_total_x variables = max allowed cells present on the grid / kind for division functions
	let global_total_TE = 17
	let global_total_ICM =  18
	let second_differentiationV = 12500							// lumen volume when second differentiation starts

	let prePercentage = 0.40											// % of cells with a PrE identity upon second differentiation event
	let allowedPre = Math.round(global_total_ICM * prePercentage)		// nr of PrE cells
	let allowedEpi = Math.round(global_total_ICM * (1 - prePercentage))	// remainder of the cells is assigned the EPI identity
	let global_total_EPI = 18											// max allowed cells when they can divide
...
//let outputTotalDisconnectedness = [], outputIcmConnectedness = []
	...
	*/
	...
	function secondDifferentiation(lumen_id){
		...
		// Conversions into PrE can only occur if the allowed percentage of PrE cells has not yet been reached
		if (icmIds.length > 0){
	...
	*/
	...
	// Compute a sorting score between EPI and PrE based on their distances to the common geometric center of the ICM. 
	// ALso measured: ICM cell shape (aspect ratio), connectedness of EPI/PrE, ICM shape, ICM disconnectedness.
	// note: this only looks at the spatial ditribution between cells, not at their angular distribution (PrE could be localized at inner part ICM, next to TE)
	// ICM shape might also affect this output.
		// Formula taken from Belousov et al., 2024 (https://journals.aps.org/prl/abstract/10.1103/PhysRevLett.132.248401)
	function sortingScore(){
		let preIds = [], epiIds = [], icmDaughters = [], totalPixels = []
		let epiDistance = {}, preDistance = {}, com, d
		for (let i of this.C.cellIDs()){
			if(this.C.cellKind(i) == 5){		
				icmDaughters.push(i)
				epiIds.push(i)
			} else if(this.C.cellKind(i) == 6){	
				icmDaughters.push(i)			
		...
		// Add all pixels from all EPI and PrE cells to one array
		for (let j of icmDaughters){
			let icmDaughterPix = this.C.getStat(CPM.PixelsByCell)[j]
			totalPixels = totalPixels.concat(icmDaughterPix)
		}
		// Calculate common geometric center (centroid) of all pixels
		...
		//console.log("Total ICM centroid is:", cvec)

		// Calculate the distance of centroids to geometric center for EPI and PrE cells
		for (let id of epiIds){
			com = this.C.getStat(CPM.Centroids)[id]
			d = Math.sqrt(Math.pow(cvec[0] - com[0], 2) + Math.pow(cvec[1] - com[1], 2)) 		// x, y distance relative to centroid
			epiDistance[id] = d
		}
			//console.log("this is the EPI distance object", epiDistance)
		for (let id of preIds){
			com = this.C.getStat(CPM.Centroids)[id]
			d = Math.sqrt(Math.pow(cvec[0] - com[0], 2) + Math.pow(cvec[1] - com[1], 2)) 		// x, y distance relative to centroid
			preDistance[id] = d
		}
			//console.log("this is the PrE distance object", preDistance)

	// Calculate the sorting index
		// Formula: Added sign of d_pre - d_epi for all combinations of distances, multiplied by 1 / (number epi cells * number pre cells)
		let sortingScore = 0, totalSign = 0
		for (const [id1, d1] of Object.entries(epiDistance)){
			for (const [id2, d2] of Object.entries(preDistance)){
				totalSign += Math.sign(d2 - d1)
			}
		}
		//console.log( "totalsign",totalSign)
		sortingScore = 1/(epiIds.length * preIds.length) * totalSign
		//console.log("At time:", sim.time, ",this is the sorting score:", sortingScore)

	// Calculate the total ICM shape
		// Loop over the pixels belonging to the ICM
		let bxx = 0, bxy = 0, byy=0, cx, cy, T, D, L2, L1, shape
		for( let j = 0 ; j < totalPixels.length ; j ++ ){
			cx = totalPixels[j][0] - cvec[0] // x position rel to centroid
			cy = totalPixels[j][1] - cvec[1] // y position rel to centroid
